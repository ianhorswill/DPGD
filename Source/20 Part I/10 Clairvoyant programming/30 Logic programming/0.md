...
Neither method matches, so both methods fail.  That means **the call itself fails**.[^1]  The command line prints "no".
...
Cat garfield.  
```
It starts by choosing `Animal` method, but depending on the method then calls either `Dog` or `Cat`.  So then it chooses a method for that.  Here's the choice tree:
```mermaid
graph TB
