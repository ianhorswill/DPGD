...
When we run `[Dog lassie]`, it works even though it doesn't print anything.  We say the call **succeeds**.  Its succeeds so long as there's a choice path through it that succeeds.  Here's the choice tree for it, and the path of the right succeeds, so the call succeeds:
